    ...
    <blockquote><p>
        <?=htmlspecialchars($joke->joketext, ENT_QUOTES, 'UTF-8')?>
            ...
            <?=htmlspecialchars($joke->getAuthor()->name, ENT_QUOTES,'UTF-8');?></a> on 
                <?php $date = new DateTime($joke->jokedate); echo $date->format('jS F Y');?>)

            <?php if ($user): ?>  
            <?php if ($user == $joke->authorid || $user->hasPermission(\Ijdb\Entity\Author::EDIT_JOKES)): ?>
            <a href="/joke/edit?id=<?=$joke->id?>">Edit</a>
            <?php endif; ?>
            <?php if ($user == $joke->authorid || $user->hasPermission(\Ijdb\Entity\Author::DELETE_JOKES)): ?>
